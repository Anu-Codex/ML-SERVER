...
const Player = mongoose.models.Player || mongoose.model('Player', new mongoose.Schema({
    name: String,
    profilePic: { type: String, default: "" }, // This will now store a URL (e.g., https://...)
...
const Tournament = mongoose.models.Tournament || mongoose.model('Tournament', new mongoose.Schema({ title: String, totalTeams: String, status: String, winner: String, joinLink: String, date: String, roster: [{ teamName: String, players: [String] }], fixtures: [{ stageName: String, matches: [{ p1: String, p2: String, s1: { type: String, default: "-" }, s2: { type: String, default: "-" } }] }] }), 'tournaments');
...
// UPDATED: ADD PLAYER (JSON URL version)
app.post('/api/add-player', async (req, res) => {
    try {
        ...
        const newP = new Player({
            name: name,
            profilePic: profilePic || "", // Stores the URL string
            wins: 0, points: 0, previousRank: 0, goalsFor: 0, goalsAgainst: 0, trophies: ""
        });

        await newP.save();
        res.json({ success: true, player: newP });
    } catch (err) {
...
app.post('/api/update-fixtures', async (req, res) => {
    ...
    try {
        ...
        if (action === 'add-stage') { tour.fixtures.push({ stageName, matches: [] }); }
            ...
            const stage = tour.fixtures.id(stageId); const match = stage.matches.id(matchId);
            const g1 = parseInt(matchData.s1) || 0; const g2 = parseInt(matchData.s2) || 0;
...
app.get('/api/arena-results', async (req, res) => {
    try {
        ...
        let allMatches = [];
        tours.forEach(tour => { if (tour.fixtures) { tour.fixtures.forEach(stage => { stage.matches.forEach(m => { allMatches.push({ ...m.toObject(), tournamentTitle: tour.title, stageName: stage.stageName, tourStatus: tour.status }); }); }); } });
